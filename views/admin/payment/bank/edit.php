...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    alertBack("ID ngân hàng không hợp lệ");
}

// Fetch bank data
$bank_id = (int)$_GET['id'];
$bank_data = $db->get_row("SELECT * FROM banks WHERE id = $bank_id");
if (!$bank_data) {
    alertBack("Ngân hàng không tồn tại");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $data_user['role'] == 1) {
    ...
    // Find bank_name corresponding to bank_code
    $bank_name = '';
    ...
    if ($db->update('banks', $data, "id = $bank_id")) {
        alertBack("Cập nhật ngân hàng thành công");
    } else {
        alertBack("Lỗi khi cập nhật ngân hàng");
    }
    $db->dis_connect();
}
$banks = [
...
<div class="content-body">
    <div class="page-titles">
        <ol class="breadcrumb">
            <li>
                <h5 class="bc-title">Chỉnh sửa ngân hàng</h5>
            </li>
            ...
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Chỉnh sửa ngân hàng</a></li>
        </ol>
    </div>
                                ...
                                <div class="row">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="bank_code" class="form-control select2" required>
                                                ...
                                                <?php foreach ($banks as $bank): ?>
                                                    <option value="<?= htmlspecialchars($bank['code']) ?>" <?= $bank_data['code'] === $bank['code'] ? 'selected' : '' ?>>
                                                        <?= htmlspecialchars($bank['name']) ?> - <?= htmlspecialchars($bank['code']) ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="account_holder" class="form-control" value="<?= htmlspecialchars($bank_data['account_holder']) ?>" required>
                                        </div>
                                    </div>
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            <label class="form-label">Số tài khoản</label>
                                            <input type="text" name="account_number" class="form-control" value="<?= htmlspecialchars($bank_data['account_number']) ?>" required>
                                        </div>
                                    </div>
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            <label class="form-label">Nội dung nạp</label>
                                            <input type="text" name="prefix_autobank" class="form-control" value="<?= htmlspecialchars($bank_data['prefix_autobank']) ?>" required>
                                        </div>
                                    </div>
                                    <div class="col-xl-6">
                                        <div class="mb-3">
                                            <label class="form-label">Token API (API.VPNFAST.VN)</label>
                                            <input type="text" name="token" class="form-control" value="<?= htmlspecialchars($bank_data['token']) ?>" required>
                                        </div>
                                    </div>
                                    <div class="col-xl-12">
