...
$total_transactions = $db->get_row("SELECT COUNT(*) as total FROM `transactions` t $where_clause")['total'];
$total_pages = ceil($total_transactions / $items_per_page);
$transactions = $db->get_list("SELECT t.*, u.username, u.email,u.image,u.lastname,u.firstname FROM `transactions` t LEFT JOIN `users` u ON t.user_id = u.id $where_clause ORDER BY t.id DESC LIMIT $offset, $items_per_page");
?>
<div class="content-body">
    <div class="page-titles">
        <ol class="breadcrumb">
            <li>
                <h5 class="bc-title">Quản lý giao dịch</h5>
            </li>
            ...
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Quản lý giao dịch</a></li>
        </ol>
                                            ...
                                            <form action="" method="GET">
                                                <div class="row">
                                                    <div class="col-12 mb-4">
                                                        <span class="text-cap text-body">Mã giao dịch</span>
                                                        <input type="text" class="form-control" name="trx_id" id="service_filter_input" value="<?= $trx_id ?>" autocomplete="off">
                                                    </div>
                                                    <div class="col-12 mb-4">
                                                ...
                                                <div class="d-grid">
                                                    ...
                                                    <a href="/admin/transaction" type="button" id="filter_button" class="btn btn-danger">Bỏ lọc</a>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                    ...
                    <div class="card-body table-card-body px-0 pt-0 pb-2 p-3">
                            ...
                            <table id="employeesTable" class="table table-borderless table-nowrap table-align-middle card-table">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th class="mw-150">MÃ GIAO DỊCH</th>
                                    ...
                                    <?php if (!empty($transactions)): ?>
                                        <?php foreach ($transactions as $transaction): ?>
                                            <tr data-id="<?= $transaction['id'] ?>">
                                                ...
                                                <td><span><?= $transaction['trx_id'] ?></span></td>
                                                <td>
                                                    <div class="d-flex">
                                                ...
                                                <td><span class="<?= $transaction['trx_type'] == '-' ? 'text-danger' : 'text-success' ?>"><?= formatCurrencyF($transaction['amount']) ?></span></td>
                        ...
                        <?php if ($total_pages > 1): ?>
                            <div class="pagination">
                                <?php if ($page > 1): ?>
                                    <a href="?page=<?php echo $page - 1; ?>&trx_id=<?= $trx_id ?>&start_date=<?= $start_date ?>&end_date=<?= $end_date ?>" class="nav-btn"><i class="fa fa-arrow-left"></i></a>
                                <?php endif; ?>
                                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                    <a href="?page=<?php echo $i; ?>&trx_id=<?= $trx_id ?>&start_date=<?= $start_date ?>&end_date=<?= $end_date ?>" class="<?php echo $i === $page ? 'active' : ''; ?>">
                                        <?php echo $i; ?>
                                    </a>
                                <?php endfor; ?>
                                <?php if ($page < $total_pages): ?>
                                    <a href="?page=<?php echo $page + 1; ?>&trx_id=<?= $trx_id ?>&start_date=<?= $start_date ?>&end_date=<?= $end_date ?>" class="nav-btn"><i class="fa fa-arrow-right"></i></a>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
