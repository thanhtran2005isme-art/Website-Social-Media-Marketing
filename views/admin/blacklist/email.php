...
<?php
// Xử lý thêm email mới
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email']) && $data_user['role'] == 1) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        alertBack("Địa chỉ email không hợp lệ!");
    }
    ...
    if ($db->insert('blacklist_emails', $data)) {
        alertBack("Thêm email vào danh sách đen thành công!", true);
    } else {
        alertBack("Lỗi khi thêm email!");
    }
}
...
$total = $db->get_row("SELECT COUNT(*) as total FROM `blacklist_emails` $where_clause")['total'];
$total_pages = ceil($total / $items_per_page);
$blacklist = $db->get_list("SELECT * FROM `blacklist_emails` $where_clause ORDER BY id DESC LIMIT $offset, $items_per_page");
?>

<div class="content-body">
    <div class="page-titles">
        <ol class="breadcrumb">
            <li><h5 class="bc-title">Danh sách đen Email</h5></li>
            <li class="breadcrumb-item"><a href="javascript:void(0)">
                ...
                Home </a></li>
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Danh sách đen Email</a></li>
        </ol>
    </div>
            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header py-3 d-sm-flex d-block align-items-center">
                        <h4 class="card-title">Danh sách đen Email</h4>
                                            ...
                                            <form action="" method="GET">
                                                <div class="row">
                                                    <div class="col-12 mb-4">
                                                        <span class="text-cap text-body">Email</span>
                                                ...
                                                <div class="d-grid">
                                                    ...
                                                    <a href="/admin/blacklist/emails" type="button" id="filter_button" class="btn btn-danger">Bỏ lọc</a>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="addBlacklistModalLabel">Thêm email vào danh sách đen</h5>
                                ...
                                <div class="modal-body">
                                    <form id="addBlacklistForm" method="POST" action="">
                                        <div class="mb-3">
                                            ...
                                            <input type="email" class="form-control" id="email" name="email" required placeholder="Ví dụ: example@domain.com">
                                        </div>
                                        <div class="mb-3">
                                            <label for="reason" class="form-label">Lý do (tùy chọn)</label>
                                            <textarea class="form-control" id="reason" name="reason" rows="4" placeholder="Nhập lý do chặn email"></textarea>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Thêm Email</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-body table-card-body px-0 pt-0 pb-2 p-3">
                        <div class="table-responsive">
                            <table id="emailsTable" class="table table-borderless table-nowrap table-align-middle card-table">
                                <thead class="table-light">
                                    <tr>
                                        <th class="mw-120">ID</th>
                                        <th class="mw-200">EMAIL</th>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="5" class="py-3 px-4 text-center text-dark">
                                                ...
                                                <p>Chưa có email nào trong danh sách đen</p>
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                        ...
                        <?php if ($total_pages > 1): ?>
                            <div class="pagination">
                                <?php if ($page > 1): ?>
                                    <a href="?page=<?php echo $page - 1; ?>&reason=<?= $reason ?>&email=<?= $email ?>&start_date=<?= $start_date ?>&end_date=<?= $end_date ?>" class="nav-btn"><i class="fa fa-arrow-left"></i></a>
                                <?php endif; ?>
                                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                    <a href="?page=<?php echo $i; ?>&reason=<?= $reason ?>&email=<?= $email ?>&start_date=<?= $start_date ?>&end_date=<?= $end_date ?>" class="<?php echo $i === $page ? 'active' : ''; ?>">
                                        <?php echo $i; ?>
                                    </a>
                                <?php endfor; ?>
                                <?php if ($page < $total_pages): ?>
                                    <a href="?page=<?php echo $page + 1; ?>&reason=<?= $reason ?>&email=<?= $email ?>&start_date=<?= $start_date ?>&end_date=<?= $end_date ?>" class="nav-btn"><i class="fa fa-arrow-right"></i></a>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                    </div>
    ...
    $(document).ready(function() {
        $('.delete-btn').on('click', function() {
            ...
            Notiflix.Confirm.show(
                'Xác nhận xóa',
                'Bạn có chắc chắn muốn xóa email này khỏi danh sách chặn?',
                'Có',
                'Không',
                function okCb() {
                        ...
                        success: function(response) {
                            ...
                            if (response.status == "success") {
                                ...
                                row.fadeOut(300, function() {
                                    $(this).remove();
                                    if ($('#emailsTable tbody tr').length === 0) {
