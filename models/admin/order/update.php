...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    die(JsonMsg('error', 'Phương thức không được hỗ trợ'));
}
...
try {
    ...
    if (isset($_POST['order_id']) && !empty($_POST['order_id'])) {
        // Single order update
        $order_id = Anti_xss($_POST['order_id']);

        // Get API credentials and api_order_id
        $credentials = getApiCredentials($order_id);
        if (!$credentials) {
        ...
        $result = $db->update(
            ...
            [
                ...
                'start_counter' => $apiResponse['start_count'] ?? '',
        ...
        // Commit transaction
        $db->commit();
        die(JsonMsg('success', 'Cập nhật đơn hàng thành công'));
    } elseif (isset($_POST['order_ids']) && is_array($_POST['order_ids']) && !empty($_POST['order_ids'])) {
        // Bulk order update
